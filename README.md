...
1. USB でキーボード接続
2. `/` キーを押しながら `N` を押す → ブートローダーモード（PC にドライブとしてマウント）
3. 取得した `meteorite40_low-xiao_ble__zmk-zmk.uf2` をドライブにドラッグ&ドロップ
4. 自動でリセット、新しいキーマップで起動

※ 初回書き込みや v1.0 時代は `/` + `Q`。書き込み済みファームウェアのバージョンで異なる。
